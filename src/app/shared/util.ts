...
export async function setBackendAndEnvFlags(flagConfig: any, backend: string) {
  ...
  for (const flag in flagConfig) {
    ...
    if (TUNABLE_FLAG_VALUE_RANGE_MAP[flag].indexOf(flagConfig[flag]) === -1) {
      throw new Error(`${flag} value is expected to be in the range [${TUNABLE_FLAG_VALUE_RANGE_MAP[flag]}], while ${flagConfig[flag]} is found.`);
    }
  }
...
function drawPath(ctx: CanvasRenderingContext2D, points: [number, number][], closePath: boolean) {
...
export function drawResults(ctx: CanvasRenderingContext2D, faces: any[], triangulateMesh: boolean, boundingBox: boolean) {
  faces.forEach((face) => {
